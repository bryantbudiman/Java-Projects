...
import messages.ClientFinishedSetUpMessage;
...
public class HostServer extends Thread{
	...
	private int clientFinishedSetUp = 0; 
	...
	public void startTimer() {
		gameTimer.schedule(new UpdateCurrentSecond(), 0, 1000);
	}
	...
	public void processMessage(Message message){
		...
		else if (message instanceof ResetBuzzedInMessage) {
			teamBuzzedIn = false; 
			sendMessage = false; 
		}
		...
		else if (message instanceof ResetGameTimerMessage) {
			currentSecond = 0;
			stopTimer = true; 
			sendMessage = false; 
		}
		else if (message instanceof StartGameTimerMessage) {
			currentSecond = 0;
			stopTimer = false; 
			sendMessage = false; 
		}
		else if (message instanceof ClientFinishedSetUpMessage) {
			clientFinishedSetUp++;
			if(clientFinishedSetUp == communicationThreads.length) startTimer();
			sendMessage = false;
		}
	...
	private class UpdateCurrentSecond extends TimerTask {
	    public void run() {
	    	if(stopTimer == false) {
