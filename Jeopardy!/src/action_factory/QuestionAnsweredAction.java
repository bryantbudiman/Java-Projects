...
public class QuestionAnsweredAction extends Action{
	...
	private void checkReadyForFJ(){
		...
		//if not ready, move on to another question
			...
			mainGUI.moveClockAnimation(gameData.getCurrentTeam().getTeamIndex());
			
			if (gameData.getCurrentTeam().getTeamIndex() != client.getTeamIndex()) mainGUI.disableAllButtons();
			
			mainGUI.getClockAnimation().resetCurrentFrame();
			mainGUI.getCountDownAnimation().setCurrentSecond(15);
			mainGUI.getClockAnimation().repaint();
			mainGUI.getCountDownAnimation().repaint();
			mainGUI.setTotalSecond(15);
	...
	private void questionDone(){
		currentQuestion.hideFishAnimation();
		...
		//if question has not been attempted by everyone, go to a buzz in period
			...
			mainGUI.getClockAnimation().resetCurrentFrame();
			mainGUI.getCountDownAnimation().setCurrentSecond(20);
			mainGUI.getClockAnimation().repaint();
			mainGUI.getCountDownAnimation().repaint();
			...
			currentQuestion.setBuzzInPeriod(gameData.getCurrentTeam().getTeamIndex());
			mainGUI.addUpdate("Buzz in to answer!");
			mainGUI.hideClocks();
			mainGUI.setBuzzIn(true);
